...
| 명령 | 동작 |
|---|---|
| `/start` | 구독 요청. 본인에게 chat_id 안내, 소유자에게 등록 알림 |
| `/test` | 전송 점검. 메시지가 닿는지 + 발송 명단에 있는지 즉시 회신 (소유자에게는 현재 수신자 수도) |

명령어 응답은 **폴링 워크플로**(`poll.yml`, 매시 23분)가 처리한다.
다이제스트 발송·Claude 호출이 없어 자주 돌려도 비용이 들지 않는다.
두 워크플로는 `concurrency: era-bot-telegram` 그룹을 공유해 `getUpdates`가 동시에 돌지 않는다.
